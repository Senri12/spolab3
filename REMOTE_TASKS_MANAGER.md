...
Из этого следует практическое правило для репозитория: локальные `src/tac_*` и `src/inspector.c` больше не нужны и не являются частью рабочего конвейера. Генератор строит `asm`, а assembler / VM / запуск даёт именно `Portable.RemoteTasks.Manager.exe`.
