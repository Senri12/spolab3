...
Теперь компилятор автоматически дописывает:
...
Это важно, потому что иначе вызовы `pipeOut`, `pipeIn`, `initTimer`, `waitForTick` не резолвятся корректно на этапе генерации ASM.
